    ...
    def run(self, state: TradingState) -> tuple[dict[Symbol, list[Order]], int, str]:
        ...
        for product in self.PRODS:
            if product == 'AMETHYSTS' or product == 'STARFRUIT':
                ...
                result[product] += orders
                # logger.print("ORDER ASdasdfasdfas",orders)
                ...
                if 'CHOCOLATE' in state.market_trades:
                    for trade in state.market_trades['CHOCOLATE']:
                        if trade.buyer == 'Vladimir' and trade.timestamp == self.time-100:
                            # buy chocolate because vladimir did
                            orders += self.calculate_orders(product, order_depth, best_ask_price, MAX_INT)

                        elif trade.seller == 'Vladimir' and trade.timestamp == self.time-100:
                            # sell chocolate because vladimir did
                            orders += self.calculate_orders(product, order_depth, -MAX_INT, best_bid_price)

            elif product == 'STRAWBERRIES':
                best_bid_price = max(order_depth.buy_orders.keys())
                best_ask_price = min(order_depth.sell_orders.keys())

                if 'STRAWBERRIES' in state.market_trades:
                    for trade in state.market_trades['STRAWBERRIES']:
                        if trade.buyer == 'Remy' and trade.timestamp == self.time-100:
                            # buy ROSES because Rihanna did
                            orders += self.calculate_orders(product, order_depth, best_ask_price, MAX_INT)

                        elif trade.seller == 'Remy' and trade.timestamp == self.time-100:
                            # sell ROSES because Rihanna did
                            orders += self.calculate_orders(product, order_depth, -MAX_INT, best_bid_price)
                ...
                if 'ROSES' in state.market_trades:
                    for trade in state.market_trades['ROSES']:
                        ...
                        elif trade.seller == 'Rhianna' and trade.timestamp == self.time-100:
                            # sell ROSES because Rihanna did
                            orders += self.calculate_orders(product, order_depth, -MAX_INT, best_bid_price)

            elif product == 'COCONUT':
                orders += self.calculate_orders(product, order_depth, 11000, MAX_INT)
                orders += self.calculate_orders(product, order_depth, -MAX_INT, 11000)
